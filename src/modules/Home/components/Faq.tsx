...
const Faq = () => {
  return (
    <Section classNames="mx-auto bg-white pb-8 lg:pb-20 flex items-center justify-center flex-col">
      ...
      <Accordion type="single" collapsible className="w-full lg:space-y-4 lg:w-[80%] text-left bg-gradient-to-b from-blue-500 to-brandColor p-8 rounded-3xl shadow-xl shadow-[#5D50C6]/40">
        {
          faq.map((item, index) => (
            <AccordionItem key={index} value={`${item.id}`}>
              <AccordionTrigger className="text-lg text-white w-full text-left">{item.question}</AccordionTrigger>
